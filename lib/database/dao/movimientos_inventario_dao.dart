...
class MovimientosInventarioDao
  ...
  MovimientosInventarioDao(AppDatabase db)
      : super(db);
...
Future<MovimientosInventarioData?> obtenerPorId(int id) {
    return (select(movimientosInventario)
      ..where((t) => t.id.equals(id)))
        .getSingleOrNull();
  }

  Future<int> insertar(
      MovimientosInventarioCompanion movimiento,
      ) {
    return into(movimientosInventario)
        .insert(movimiento);
  }

Future<bool> actualizar(
MovimientosInventarioData movimiento,
      ) {
    return update(movimientosInventario)
        .replace(movimiento);
  }

  Future<int> eliminar(int id) {
    return (delete(movimientosInventario)
      ..where((t) => t.id.equals(id)))
        .go();
  }
}